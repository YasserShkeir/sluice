...
- **Reads only your own, already-authenticated session, on your own machine.** It injects nothing into the target service; it cannot see a channel you can't already see, or act as anyone but you.
...
- **ToS gray area.** Programmatically capturing/scripting a client and calling its endpoints with a *session* token (e.g. Slack's `xoxc`) is **against that service's Terms of Service**, even though the account is yours, the machine is yours, and nothing leaves the box. Session tokens are not an authorized programmatic auth mechanism. This is the same class of thing as a browser extension that automates your own logged-in session. Sluice does not pretend otherwise.
- **Credential blast radius.** A session token **plus** its session cookie together equal your logged-in session — anyone who obtains both can read and *act as you* until the session is revoked. That is exactly why Sluice keeps them in memory and never writes or transmits them.
- **Real risks to your account.** Rate-limiting, session invalidation, and — at an admin's or the service's discretion — account flagging or suspension. "Local-only data" protects the *captured data*; it does **not** make the *access* invisible to the service's audit logging.
- **Org policy.** Your employer's acceptable-use policy may forbid this even on your own account/machine. Check first — Sluice can't adjudicate that for you.
...
- **Replay cannot write.** Replays are checked below every caller (CLI, web UI, MCP): mutating HTTP verbs are refused, a denylist blocks write/admin operations (`chat.postMessage`, `admin.*`, …), and a shared rate budget plus single-flight concurrency bound what Sluice can do to your account. A modified frontend cannot route around this.
